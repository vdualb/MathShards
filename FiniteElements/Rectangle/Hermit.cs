...
{
    // 16 функций
    public static Func<PairReal, Real> BasisTemplate(int i)
    {
        int mu = 2*(i/4%2) + i%2;
        int nu = 2*(i/8) + i/2%2;
    ...
    public static Real BasisConverted(int i, PairReal p0, PairReal p1, PairReal p)
    {
        int mu = 2*(i/4%2) + i%2;
        int nu = 2*(i/8) + i/2%2;
    ...
    /// xy==0 => diff x
    /// xy==1 => diff y
    public static Func<PairReal, Real> BasisGradTemplate(int i, int xy)
    {
        int mu = 2*(i/4%2) + i%2;
        int nu = 2*(i/8) + i/2%2;
    ...
    /// xy==0 => diff x
    /// xy==1 => diff y
    public static Real BasisGradConverted(int i, int xy, PairReal p0, PairReal p1, PairReal p)
    {
    ...
    where Tc : ICoordSystem
    {
        ...
        for (int i = 0; i < sd; i++)
        {
            for (int j = 0; j < sd; j++)
            {
                ...
                var funcStiffness = (PairReal point) => {
